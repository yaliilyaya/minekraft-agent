function AgentCraft () {
  ...
  this.findAllRecipe = async (name, useCraftingTable) => {
    let recipe = this.AgentRecipeInfo.findAllRecipe(null, name)

    if (useCraftingTable && recipe.isEmpty()) {
      let craftingTable = await this.AgentFinder.findCraftingTable()
      return this.AgentRecipeInfo.findAllRecipe(craftingTable, name)
    }
...
module.exports = {
  AgentCraftBuilder: (bot, AgentBlockInfo, AgentFinder) => {
