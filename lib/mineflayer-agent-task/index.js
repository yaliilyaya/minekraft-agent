  ...
  this.runTaskDigBlocks = async (blockName, count = 1) => {
    ...
    if (blockIds.length > 0) {
      for (const number of Enumerable.range(0, count)) {
        ...
        const blocks = this.AgentFinder.findBlocksIdByIdInRange(blockIds, [8, 10, 20, 50, 100, 500, 1000], startPosition)

        const block = this.AgentFinder.findFirstBlock(blocks, startPosition)
        ...
        if (block !== undefined) {
          ...
          //TODO:: нужно экипировывать инструмент согласно добываемым блокам
          await this.AgentInventory.equipTool('shovel');
...
module.exports = {
  AgentTaskBuilder: (bot, AgentBlockInfo, AgentDig, AgentInventory, AgentFinder) => {
