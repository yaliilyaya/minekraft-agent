...
function AgentTask () {
  ...
  this.craftItemInInventory = async (name, amount = 1) => {
    this.AgentCraft.craftItemInInventory(name, amount);
  }

  this.craftItemInCraftingTable = async (name, amount = 1) => {
    //TODO:: нужно подойти к верстаку для того чтобы что то создать
    this.AgentCraft.craftItemInCraftingTable(name, amount);
  }
