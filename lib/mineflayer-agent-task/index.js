...
function AgentTask () {
  ...
  this.craftItem = async (name, amount = 1) => {
    ...
    const recipeCollection = await this.AgentCraft.findAllRecipe(name, false)

    const recipes = recipeCollection.recipes.filter( recipe => {
      let recipeItems = recipe.delta.filter(recipeItem => recipeItem.count < 0)
    ...
    const isSuccess = await this.AgentCraft.craft(recipe, craftCount, false);
    if (isSuccess) {
