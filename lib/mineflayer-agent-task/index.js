...
function AgentTask () {
  ...
  this.run = async (task, callback) => {
    ...
    console.log('Что то не так. начинаю создание - ' + task.toString())
    callback({type: 'error'});
    return;

    switch (task.type) {
      case 'crateItem': {
        await this.craftItem(task.parts[0].name, task.parts[0].count)
  ...
  this.craftItem = async (name, amount = 1) => {
    ...
    const recipe = await this.AgentCraft.findRecipe(name, false)
    if (!recipe) {
      return false
    }
    await this.AgentCraft.craft(recipe, amount, false);
